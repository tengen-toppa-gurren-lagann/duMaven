...
internal class DiskUsageTest {
    ...
    fun calcSizeAndWriteResult() {
        val files1 = listOf<String>() // Список файлов пуст
        var result = DiskUsage(files1).calcSizeAndWriteResult(readable = false, summed = false, si = false) // Список файлов пуст -> ошибка
        assertEquals(1, result)

        val files2 = listOf<String>("input/Test") // Существующая папка -> успех
        result = DiskUsage(files2).calcSizeAndWriteResult(readable = false, summed = false, si = false)
